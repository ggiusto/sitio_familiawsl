...
def get_node_data(member, level=0):
    ...
    Permite la visualización de descendencia completa (hasta Choznos y más allá).
    """
    ...
    if member.foto_principal:
        try:
            # Verificamos que el archivo exista para evitar errores de renderizado
    ...
    if member.conyuge:
        ...
        nombre_c = member.conyuge.first_name if member.conyuge.first_name else ""
        apellido_c = member.conyuge.last_name_paterno if member.conyuge.last_name_paterno else ""
        conyuge_nombre = f"{nombre_c} {apellido_c}".strip()
    ...
    # 3. RECURSIÓN PARA DESCENDIENTES (SIN LÍMITE DE NIVEL)
    children_list = []
    
    # Buscamos todos los miembros que tengan a este miembro actual como su padre O su madre.
    # .distinct() es crucial para no duplicar hijos si ambos progenitores están en el sistema.
    hijos = Member.objects.filter(
        Q(padre=member) | Q(madre=member)
    ).distinct().order_by('birth_date')

    for child in hijos:
        # Llamada recursiva: Cada hijo ejecuta de nuevo esta función buscando a sus propios hijos.
        # Esto garantiza que si un Bisnieto tiene hijos (Tataranietos) y estos a su vez tienen 
        # hijos (Choznos), todos sean incluidos en la lista 'children'.
        children_list.append(get_node_data(child, level + 1))

    # 4. ESTRUCTURA DE RETORNO (JSON para D3.js)
    node_data = {
        "name": member.first_name if member.first_name else f"Miembro {member.pk}",
        "last_name": member.last_name_paterno if member.last_name_paterno else "",
        "pk": member.pk,
        "level": level,  
        "generacion": member.get_relationship_display() if hasattr(member, 'get_relationship_display') else f"Gen {level}",
        "foto": foto_url, 
    ...
    # D3.js requiere que la clave 'children' exista solo si hay elementos.
    # Si 'children_list' está vacía, el nodo se considera una "hoja" (el fin de esa rama).
    if children_list:
...
def export_member_pdf(request, pk):
    ...
    # 1. Obtención encadenada de generaciones
    # Hijos
    hijos = Member.objects.filter(
        Q(padre=member) | Q(madre=member)
    ).distinct().order_by('birth_date')
    ...
    # Limpiamos el nombre del archivo para evitar problemas con espacios o tildes
    filename = f"Ficha_{member.last_name_paterno or 'Miembro'}_{member.pk}.pdf"
    response['Content-Disposition'] = f'attachment; filename="{filename}"'
    
    # base_url es fundamental para que WeasyPrint encuentre las imágenes/estáticos

...
def get_node_data(member, level=0):
    ...
    Muestra el nombre completo del titular y el del cónyuge si existiera.
    """
    
    # 1. Definir el nombre a mostrar (Titular)
    nombre_mostrar = member.first_name if member.first_name else f"Miembro {member.pk}"
    
    # 2. Lógica para incluir al Cónyuge en el mismo nodo
    if member.conyuge:
        nombre_mostrar += f" & {member.conyuge.first_name}"

    # 3. Obtener URL de la foto de perfil de forma segura
    foto_url = None
    ...
    # 4. Recursión para descendientes (Hijos)
    # Buscamos miembros donde este miembro sea padre O madre
    children_list = []
    ...
    # 5. Retorno de datos para el script D3.js
    return {
        "name": nombre_mostrar,  # Aquí es donde se envía la unión de la pareja
...
def member_tree_data(request):
    ...
    """
    # Cambiar 'parent' por 'padre' y 'madre'
